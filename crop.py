        ...
        if scan_excel_files:
            ...
            if response:
                ...
                for i, f in enumerate(scan_excel_files):
                    button = tk.Button(outer_frame, text=f"{i + 1}. {f}", width=30, command=lambda i=i: select_file(i))
                    button.pack(pady=0)
...
if crop_mode == 1 or crop_mode == 2:
    if not os.path.isfile(annotation_file_path):
        ...
        #annotation_file_path = input(f"Enter the path to the {file_type[(crop_mode-1)]} file: ")
...
def process_OCR_text(detected_text, frame):
    ...
    if re.match(correct_format, detected_text[-8:]):
        ...
        dialog_height = img_frame_height // 4
        ...
        if (frame_count % frame_skip == 0):
            for i, point in enumerate(points_of_interest_entry[index]):
                if cropped_frames == 1:
                    ...
                    # save file
                    cv2.imwrite(f"./{output_folder}/{species}_{timestamp}_{frame_number_start + frame_count}_{crop_counter}_{x1},{y1}_{x2},{y2}.jpg", crop_img)
            if whole_frame == 1:
                cv2.imwrite(f"./{output_folder}/whole frames/{species}_{timestamp}_{frame_number_start + frame_count}_{crop_counter}_whole.jpg", frame)
            crop_counter += 1
...
# Set path to folder containing video files
folder_path = video_folder_path

# Create list to hold first frames of each video
frames = []
...
#Create LEFT ARRROW Button
left_arrow = tk.Canvas(toolbar, width=50, height=50)
left_arrow.pack(side=tk.LEFT)
left_arrow.create_polygon(10, 20, 30, 10, 30, 30, fill='black')

left_button = tk.Button(toolbar, text="Previous folder", padx=10, pady=5, height=3)
left_button.pack(side=tk.LEFT)

#Create RIGHT ARROW Button
right_arrow = tk.Canvas(toolbar, width=50, height=50)
right_arrow.pack(side=tk.RIGHT)
right_arrow.create_polygon(10, 10, 30, 20, 10, 30, fill='black')

right_button = tk.Button(toolbar, text="Next folder", padx=10, pady=5, height=3)
...
save_button = tk.Button(toolbar, text="Save", image=save_icon, compound=tk.LEFT, padx=10, pady=5, height=48)
save_button.pack(side=tk.LEFT)

on_image = tk.PhotoImage(width=116, height=58)
off_image = tk.PhotoImage(width=116, height=58)
on_image.put(("green",), to=(0, 0, 57,57))
off_image.put(("red",), to=(58, 0, 115, 57))
var1 = tk.IntVar(value=1)
cb1 = tk.Checkbutton(toolbar, image=off_image, selectimage=on_image, indicatoron=False,
                     onvalue=1, offvalue=0, variable=var1)
cb1.pack(side=tk.LEFT, padx=(500,0), pady=0)
auto_button = tk.Button(toolbar, text="\t\tAutomatic evaluation\t\t", padx=10, pady=5, height=3)
...
for i in range(rows):
    ...
    if (((i) * 6)+6) + 1 <= len(video_filepaths):
        ...
        hour_2nd = len(video_filepaths) % 6
...
for i in range(rows):
    ...
    for j in range(6):
        if (j + ((i) * 6)) < len(video_filepaths):
            ...
            dummy_frame = tk.Button(button_frame, image=new_img, foreground="white")
...
#
# # If the frame was successfully read, open it and prompt the user to click on the first point of interest
# if cropped_frames == 1:
#     # Load the video and read the first frame of the video
#     cap = cv2.VideoCapture(video_filepaths[0])
#     success, frame = cap.read()
#     if success:
#         cv2.imshow("Frame", frame)
#         cv2.setWindowProperty("Frame", cv2.WND_PROP_TOPMOST, 1)
...
#         # Ask the user if they want to select additional points of interest
#         while True:
#             add_point = input("Do you want to add another point of interest? (y/n) ")
#             if add_point.lower() == "y":
#                 # Draw a rectangle around the already selected points of interest
...
#                 cv2.setMouseCallback("Frame", get_mouse_position)
#                 cv2.waitKey(0)
#             elif add_point.lower() == "n":
#                 break
#     print(points_of_interest)
points_of_interest = points_of_interest_entry
...
while True:
    run_sorting = input("Do you want to run the sorting script on the generated images? (y/n)").lower()
    if run_sorting in ['y', 'n']:
        break
    else:
        print("Invalid input. Please enter 'y' or 'n'.")

# Run sorting script if user wants to
if run_sorting == 'y':
